...
test.describe('RPN Calculator - Display', () => {
  ...
  test('初期状態でスタック値が正しく表示される', async ({ page }) => {
    ...
    await expect(stackValues.nth(0)).toHaveText('0')
    await expect(stackValues.nth(1)).toHaveText('0')
    await expect(stackValues.nth(2)).toHaveText('0')
    await expect(stackValues.nth(3)).toHaveText('0')
  })
  ...
  test('Drop操作後のスタック表示が正しい', async ({ page }) => {
    ...
    await expect(page.locator('.stack-item').nth(1).locator('.stack-value')).toHaveText('0') // Z
